...
func DefaultDataDir() string {
	...
	if home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "P202")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Local", "P202")
		} else {
...
func FixDefaultDataDir(prevDataDir, curDataDir string) error {
	...
	if !prevDataDirExists {
		// If no previous "%APPDATA%\P202" found, nothing to patch and move to new default location.
		return nil
	}
